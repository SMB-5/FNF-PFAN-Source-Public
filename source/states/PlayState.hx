...
class PlayState extends MusicBeatState
{
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			stagesFunc(function(stage:BaseStage) stage.countdownStart());
	...
	private function generateSong():Void
	{
		...
		{
			if (section.changeBPM != null && section.changeBPM == true && section.bpm != null && daBpm != section.bpm) {
				daBpm = section.bpm;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if ((ClientPrefs.data.comboBreak || merciless) && (daRating.name == 'bad' || daRating.name == 'shit'))
		{
			combo = 0;
			makeGhostNote(note);
			if(char != gf && lastCombo >= 50 && gf != null && gf.animOffsets.exists('sad'))
			{
				gf.playAnim('sad');
				gf.specialAnim = true;
			}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (end != null && end.extraData.get('holdSplash') != null) {
			end.extraData.get('holdSplash').visible = false;
		}
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(char != null && (note == null || !note.noMissAnimation) && char.hasMissAnimations)
		{
			...
			if(char != gf && lastCombo > 49 && gf != null && gf.hasAnimation('sad'))
			{
				gf.playAnim('sad');
				gf.specialAnim = true;
			}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.hitCausesMiss) //Common notes
		{
			...
			if (!opponentMode)
			{
				...
				if (char != gf && gf != null && !note.isSustainNote)
				{
	...
	public function spawnHoldSplashOnNote(note:Note) {
		if (!note.isSustainNote && note.tail.length != 0 && note.tail[note.tail.length - 1].extraData.get('holdSplash') == null) {
			spawnHoldSplash(note);
		}
		else if (note.isSustainNote) {
			var end:Note = note.animation.curAnim.name.endsWith('end') ? note : note.parent.tail[note.parent.tail.length - 1];
			if (end != null) {
				var leSplash:SustainSplash = end.extraData.get('holdSplash');

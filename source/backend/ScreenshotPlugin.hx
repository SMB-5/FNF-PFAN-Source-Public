...
class ScreenshotPlugin extends FlxBasic
{
  ...
  {
    return Controls.instance.SCREENSHOT && !noSavingScreenshots;
  }

  public function updateFlashColor():Void
  {
    ClientPrefs.data.flashing ? set_flashColor(FlxColor.WHITE) : null;
  }
  ...
  {
    if (!PREVIEW || screenshotBeingSpammed && !ClientPrefs.data.flashing || stateChanging) return; // Sorry, the previews' been cancelled
  ...
  function saveBufferedScreenshots(screenshots:Array<Bitmap>, screenshotNames)
  {
    ...
    if (!ClientPrefs.data.flashing && !PREVIEW_ONSAVE)

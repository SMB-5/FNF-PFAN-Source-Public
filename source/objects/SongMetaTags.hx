...
class SongMetaTags extends FlxSpriteGroup
{
    ...
    public function new(_x:Float, _y:Float, _song:String) {
        ...
        var text = new FlxText(0, 50, 0, "", fontSize);
        text.setFormat(Paths.font("whyvcr.otf"), fontSize, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);

        text.text = CoolUtil.getText(Paths.txt(_song.toLowerCase() + "/meta"));

...
class TouchButton extends TypedTouchButton<FlxSprite>
{
	 ...
	 * @param   IDs     The binds to use for inputs.
	 */
	public function new(X:Float = 0, Y:Float = 0, ?IDs:Array<MobileInputID>)
	{
		super(X, Y, null, IDs);
	}
}
...
class TypedTouchButton<T:FlxSprite> extends FlxSprite implements IFlxInput
{
	...
	/**
	* Whether to allow automatically changing the label's alpha
	*/
	public var changeLabelAlpha:Bool = true;
	...
	/**
	* The IDs assigned to this button.
	*/
	...
	*/
	 ...
	 * @param   IDs    The binds to use for inputs.
	 */
	public function new(X:Float = 0, Y:Float = 0, ?OnClick:Void->Void, ?IDs:Array<MobileInputID>)
	{
		super(X, Y);
	...
	/**
	 * Basic button update logic - searches for overlaps with touches
	 * and calls `updateStatus()`.
	 */
	function updateButton():Void
	{
		// We're looking for any touch overlaps with this button
	...
	{
		if (_spriteLabel != null && changeLabelAlpha && labelAlphas.length > status)
		{

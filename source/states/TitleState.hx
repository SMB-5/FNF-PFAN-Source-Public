...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		titleText.frames = Paths.getSparrowAtlas(#if !mobile 'titleEnter' #else 'titleEnter_mobile' #end);
		titleText.animation.addByPrefix('idle', "ENTER IDLE", 24);
		titleText.animation.addByPrefix('press', ClientPrefs.data.flashing ? "ENTER PRESSED" : "ENTER FREEZE", 24);
		titleText.animation.play('idle');
		titleText.updateHitbox();
		#if mobile titleText.x -= 30; #end
		add(titleText);
	...
	override function update(elapsed:Float)
	{
		...
		var pressedEnter:Bool = FlxG.keys.justPressed.ENTER || controls.ACCEPT #if mobile || TouchUtil.justPressed #end;
